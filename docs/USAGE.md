...
4. 点**确定**。源一旦可见（预览或直播）就会自动开始播放
   - 若没自动开始，点属性里的 **「▶ 立即切换 / Play now」** 按钮强制起播
...
源属性面板顶部有两行**实时信息**（每秒刷新）：

```
当前播放   my_movie.mkv  [3/12]  42s / 1450s  (2%)
会话       1920x1080 · SDR · low-mem
```

- `[3/12]` = 播放列表第 3 项、共 12 项
- 直播流没有时长，显示 `(直播 / 未知时长)`
- 第二行是插件识别到的 OBS 配置：画布尺寸 · HDR/SDR 会话 · 当前内存模式
