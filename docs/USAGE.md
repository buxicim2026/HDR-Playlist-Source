...
属性面板是**纯静态配置**，不含任何实时状态/预览行。

> 本版已移除「当前播放」实时信息行：它需要每秒调用一次
> `obs_source_update_properties()` 重建整个属性面板，既是"鼠标滚到底部又弹回
> 「交叉淡化转场」"的原因，也带来每秒一次的 UI 重建开销。播放进度请看 OBS 的
> 媒体控制条（时长/进度），播放状态请看日志里的 `[HDR-PL] hb:` 心跳行。
