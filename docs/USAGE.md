...
> 需要 **OBS Studio 30 或更新版本**；30 以下版本兼容性未知（插件会对低于 30.0.0 的版本打印警告）。
...
面板顶部有两行**当前媒体信息**（**只在打开面板与切换片段时更新，不按秒刷新**，
所以不会再出现"滚到哪儿都被顶回『交叉淡化转场』"的情况）：
...
- 第二行：OBS **画布分辨率 @ 帧率** · **输出色彩空间** · 是否 HDR 会话
- 说明：OBS 的插件 API 不暴露媒体文件自身的帧率与"是否隔行"标记，因此这里显示的是**输出帧率**和**插件正在使用的去隔行设置**
- 面板底部是本插件署名与 **「支持我们」** 按钮（点击调用系统默认浏览器打开赞助页）

播放进度请看 OBS 的媒体控制条；播放状态看日志里的 `[HDR-PL] hb:` 心跳行。
