...
5. 属性面板是纯静态配置（无实时读数、无每秒刷新）；属性窗口**顶部那块会动的画面是 OBS 自带的预览控件**，对所有视频源都会显示，插件无法关闭，但它不会额外解码。
...
- **没有声音**：本版改为 **libobs 原生推流模型**——在子源的音频捕获回调里直接 `obs_source_output_audio()` 推给本插件源，缓冲/对齐/重采样/时间戳全部交给 OBS（所有媒体源都走这条路），不再注册 `audio_render` 回调（它要求源自己持有时钟与缓冲，时间戳一落在混音窗口之外，libobs 就会静默丢弃整块音频）。日志里 `[HDR-PL] hb:` 的 `push=<frames>/5s` 可确认是否在推：48kHz 立体声正常约 240000/5s。（`obs_source_get_audio_mix` + `obs_source_get_audio_timestamp`，与场景处理普通媒体源的方式完全一致），采样率/时间戳/声道对齐全部交给 OBS。若仍无声，请把 OBS 日志（帮助 → 日志文件）里含 `[HDR-PL]` 的片段发我，心跳行 `audio=2ch fill=…` 会直接指出是哪一侧没有数据。
...
- **v0.0.3**（当前正式版）：
  - **交叉淡化特效没被打包（硬切真因）**：打包脚本只拷贝了 `data/locale`，`data/effects/crossfade.effect` 根本没进压缩包，运行时 `obs_module_file()` 找不到文件 → 特效加载失败 → 只能硬切。现在三个平台都会打包 `data/effects`，并且特效编译失败时会把**真实错误信息**写进日志；同时把 effect 里的"三元表达式/辅助函数"等易触发编译失败的写法去掉，只留最简单的 `lerp`
  - **切换时最后一帧比例错乱**：`fit_into()` 之前用**下一个视频**的尺寸去拟合**上一个视频**（两者分辨率不同就错位）。现在对上一条/下一条分别计算各自的贴合比例；另外子源媒体尚未打开时（宽高为 0）不再按 1:1 硬画，避免首个画面被放大
  - **播完不留白**：播放完毕（或停止/隐藏）后不再绘制上一帧，画面回到下层（空白）
  - **音频（再次定位真因）**：改为 libobs 原生推流模型（`obs_source_output_audio`，与所有媒体源同一条路径），移除 `audio_render` 回调——该回调要求源自己持有时钟与缓冲，时间戳落在当前混音窗口之外时 libobs 会静默丢弃整块音频，这就是"永远没声音"的根因。捕获回调的锁顺序问题也保持修复（注册/注销都在锁外）
  - **属性面板保持纯静态**：不显示实时进度、也不每秒刷新（避免整块面板重建把滚动位置顶回）；属性窗口顶部那块会动的画面属于 OBS 自带预览（`OBS_SOURCE_VIDEO` 输入源的固有行为），插件无法关闭
